...
export class OrderModule {
  ...
  setbillamount() {
    ...
    this.BillAmount +=
      this.TaxAmount + this.extra + this.Charges - this.OrderTotDisc - this.AllItemTotalDisc
    if (this.DiscType == 1) {
      this.DiscPercent = 0
    }
...
export class OrderItemModule {
  ...
  constructor(product, options, showname) {
    console.log(product, options, showname)
    this.DiscType = 1
...
export class KOTModule {
  ...
  constructor(items: Array<OrderItemModule>, kotno) {
    ...
    items.forEach(item => {
      item.Quantity = item.Quantity - item.kotquantity
